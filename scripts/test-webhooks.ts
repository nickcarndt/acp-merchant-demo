...
async function runTests() {
  ...
  const afterUpdate = await CheckoutService.getCheckout(checkout.checkout_id);
  ...
  await CheckoutService.markProcessing(checkout.checkout_id, 'pi_test_123456789');
  const afterProcessing = await CheckoutService.getCheckout(checkout.checkout_id);
  ...
  await CheckoutService.markCompleted(checkout.checkout_id, orderId);
  const afterCompleted = await CheckoutService.getCheckout(checkout.checkout_id);
  ...
  const failCheckout = await CheckoutService.createCheckout({
    checkout_reference_id: 'fail_test_ref',
    line_items: [{ product_id: 'prod_water_bottle', quantity: 1 }],
  });
  await CheckoutService.markFailed(failCheckout.checkout_id, 'Card declined');
  const afterFailed = await CheckoutService.getCheckout(failCheckout.checkout_id);
